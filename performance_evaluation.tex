...
The production data from PV plants consists of hourly aggregated production data of eight PV plants:
\begin{enumerate}
...
The evaluation methodology is based on two relevant error metrics: MAPE and MAE.
...
Another technique, reported in \cite{Cerqueira2020} is the holdout approach repeated in multiple testing periods using a Monte Carlo simulation.
...
This evaluation methodology is used at training time to study the performance over time.
...
The aggregated consumption data over the customers is analyzed to get some descriptive analytics before finding adequate models for forecasting the customers' demand.
...
Aggregating the data on a daily basis it is possible to observe more clearly the weekly pattern of consumption, which exhibits an increase on weekdays and a decrease on weekends.
...
The basic data is enhanced with the air temperature, the apparent temperature, and the relative humidity since they are considered the only weather features capable of influencing customers' electricity consumption.
...
The GRU model is designed as a 3-layer model with 24 Bidirectional GRU units in the first layer which use ReLU activation function and sigmoid as recurrent activation function with both dropout and recurrent dropout of 0.02.
...
The number of splits used for the blocked k-fold cross-validation is 12 with a size of a month for each test split, namely 30 days of data.
...
\hline
\end{tabular}
\caption{Summary table of the results in electricity demand forecasting with an hourly granularity.}
...
\hline
\end{tabular}
\caption{Summary table of the results in electricity demand forecasting with an hourly granularity at forecasting time on the last split.}
...
\hline
\end{tabular}
\caption{Summary table of the results in electricity demand forecasting with a daily granularity.}
\label{tab:demanddailyresults}
\end{table}

For the daily granularity, the models start with 249 entries for training and predict every time 30 entries until reaching the last prediction instant where the model has a training size of 579.
...
\hline
\end{tabular}
\caption{Summary table of the results in electricity demand forecasting with a daily granularity at forecasting time on the last split.}
...
The daily mean percentage of production data is reported in figure~\ref{fig:productiondataplotday}.
Aggregating the data on a daily basis it is possible to observe that there is no pattern in the PV plants’ production over the days.
...
\end{itemize}
It can be noticed that both the coefficients indicate a moderate to strong correlation between the hourly mean percentage of production and several weather features, including the air temperature, the apparent temperature, the relative humidity, the diffuse horizontal irradiance, the direct normal irradiance, the global horizontal irradiance, the solar radiation, the UV index, and the solar elevation angle.
...
\hline
\end{tabular}
\caption{Summary table of the results in electricity production forecasting with an hourly granularity.}
...
\hline
\end{tabular}
\caption{Summary table of the results in electricity production forecasting with an hourly granularity at forecasting time on the last split.}
...
\begin{minipage}[b]{8.5cm}
\centering
\includegraphics[width=0.83\textwidth]{images/baseline/ft_hour_week_customer1}
\subcaption{First customer.}
...
\begin{minipage}[b]{8.5cm}
\centering
\includegraphics[width=0.83\textwidth]{images/baseline/ft_hour_week_customer2}
\subcaption{Second customer.}
\label{fig:ftcustomer2}
\end{minipage}
\begin{minipage}[b]{17cm}
\centering
\includegraphics[width=0.415\textwidth]{images/baseline/ft_hour_week_customer3}
\subcaption{Third customer.}
...
Aggregating the data on a daily basis it is possible to observe a weekly pattern of consumption, which is more visible looking at the daily consumption data of the second customer.
...
\begin{minipage}[b]{8.5cm}
\centering
\includegraphics[width=0.83\textwidth]{images/baseline/ft_day_customer1}
\subcaption{First customer.}
...
\begin{minipage}[b]{8.5cm}
\centering
\includegraphics[width=0.83\textwidth]{images/baseline/ft_day_customer2}
\subcaption{Second customer.}
\label{fig:ftdaycustomer2}
\end{minipage}
\begin{minipage}[b]{17cm}
\centering
\includegraphics[width=0.415\textwidth]{images/baseline/ft_day_customer3}
\subcaption{Third customer.}
...
The results show that the daily consumption data of the three customers presents:
\begin{itemize}
...
The SARIMA model considers the week as the period for seasonal differencing since data presents a good correlation with that time instants and the model can try to take advantage of the weekly seasonality.
...
The CNN model is designed as a 2-layer model with a first layer composed of 32 1D Convolutional units with a kernel size of 5 which uses ReLU activation function.
A 1D max polling operation is applied before entering the second layer composed of 16 1D Convolutional units with a kernel size of 3 which use ReLU activation function.
A 1D max polling operation is applied before the flattening operation and entering the final dense unit to output the final prediction.
The model is trained using a combination of MAE and MAPE as loss and Nadam as optimizer with a learning rate of 0.005.
...
% \hline
% XGBoostRegressor & 117.86 $\pm$ 31.29 | 0.157 $\pm$ 0.038 & 120.87 | 0.067\\
% \hline
...
% \hline
% \end{tabular}
% \caption{Summary table of the results in consumption baseline forecasting for the first customer with an hourly granularity.}
...
\hline
\end{tabular}
\caption{Summary table of the results in consumption baseline forecasting for the second customer with an hourly granularity.}
\label{tab:baselinehourlyresults}
\end{table}
...
% \hline
% XGBoostRegressor & 121.36 $\pm$ 34.22 | 0.184 $\pm$ 0.068 & 103.16 | 0.161\\
% \hline
...
% \hline
% \end{tabular}
% \caption{Summary table of the results in consumption baseline forecasting for the third customer with an hourly granularity.}
% \label{tab:baselinehourlyresults}
% \end{table}
...
% \hline
% XGBoostRegressor & 50.01 $\pm$ 31.96 | 2.597 $\pm$ 0.890 & 89.73 | 1.939\\
% \hline
...
% \hline
% \end{tabular}
% \caption{Summary table of the results in consumption baseline forecasting for the first customer with a daily granularity.}
...
\hline
\end{tabular}
\caption{Summary table of the results in consumption baseline forecasting for the second customer with a daily granularity.}
\label{tab:baselinedailyresults}
\end{table}
...
% \hline
% XGBoostRegressor & 50.11 $\pm$ 56.39 | 2.027 $\pm$ 1.003 & 75.51 | 3.045\\
% \hline
...
% \hline
% \end{tabular}
% \caption{Summary table of the results in consumption baseline forecasting for the third customer with a daily granularity.}
% \label{tab:baselinedailyresults}
% \end{table}
