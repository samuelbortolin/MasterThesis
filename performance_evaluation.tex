...
The model is trained using mean absolute percentage error as loss and Nadam (Adam with Nesterov momentum) as optimizer with a learning rate of 0.005.
...
A 1D max polling operation is applied before entering the third dense layer composed of 4 units with ReLU activation function before entering the final dense unit to output the final prediction.
The model is trained using mean absolute percentage error as loss and Nadam (Adam with Nesterov momentum) as optimizer with a learning rate of 0.005.

The combination of the previous techniques was performed by combining one-day baseline, one-week baseline, and prophet with possibly one of LSTM, GRU, or CNN with the intent of triying to improve the robustness of DL models with some stable baselines and prophet model which internally automatically deals with seasonalities.
...
% TODO sortare per mape e capire quele dei 2, + spiegare differenze ranking, deep dive su lstm 
% perche alcuni modelli di ml vanno meglio di modelli di dl più sofisticati, magari con meno dati rete piu complessa fa fatica
...
% ottimizzare la combinazione (a trainining e poi vedere a forecasting), maybe LR on the models on a validation set instead of just the mean
...
Instead, the AutoML approach obtained a one-week MAPE of 10.61 and a one-month MAPE of 20.98 after a total run of 10 hours.
