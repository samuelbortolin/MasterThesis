...
\begin{itemize}
  \item Data preprocessing, which retrieves the needed data from the data DB, aggregates it correctly, cleans it by filling the gaps by linear interpolation, and enriches it using the weather information retrieved from the weather DB. For future data forecasts in addition to past weather data also weather forecasts are collected;
...
% TODO: describe the specialization of what is written in the architecture section
...
\begin{figure}[H]
\centering
\includegraphics[width=0.2\textwidth]{images/demand_prediction_uml}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.2\textwidth]{images/baseline_uml}
...
\centering
\includegraphics[width=0.4\textwidth]{images/single_plant_uml}
\caption{The UML data class for a single plant for electricity production forecasting.}
\label{fig:umlsingleplant}
\end{minipage}
\ \hspace{2mm} \
\begin{minipage}[b]{8.5cm}
\centering
\includegraphics[width=0.4\textwidth]{images/production_prediction_uml}
\caption{The UML data class for the aggregated data over plants for electricity production forecasting.}
