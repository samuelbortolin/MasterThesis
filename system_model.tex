...
\begin{itemize}
  ...
  \item Forecast future data for a specific use case using a specified model or the latest model for a certain time granularity, for a certain starting date and time horizon.
\end{itemize}
...
\begin{itemize}
  ...
  \item Forecasting task: forecasts future data using a specified model or the latest model;
...
The identifier of the task is then returned to the user, who can request the status of the data loading request and check the result of the operation.
...
The model training task is described more in detail in the next subsection.
...
The models training module takes the preprocessed data and trains the models with the provided specifications.
...
The prediction module takes the preprocessed data and computes the forecasts.
...
The diagram representing the performance evaluation flow is presented in figure~\ref{fig:schedulerflow}.
Describe it ...
...
Describe Data Parser, Data Preprocessing and Enrichment, Model Training, Prediction, Performance evaluation, ...
